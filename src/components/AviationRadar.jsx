...
export default function AviationRadar({ center, initialRadius, onRadius, initialSel, initialSelLabel = null, initialSelSeen = null, onSelect, defaultRadius = 100 }) {
  ...
  useEffect(() => {
    ...
    acRef.current = BACKEND_URL ? {} : seedSim(center.lat, center.lng, radius);
    if (BACKEND_URL) { setStatus("connecting"); liveRef.current = false; }
          ...
          return (
            <g key={a.id} transform={`translate(${a.x} ${a.y})`} onClick={() => setSel(a.id)} style={{ cursor: "pointer" }}>
              ...
              {a.isDrone ? (
                ...
                <g transform={`rotate(${a.headingDeg || 0}) scale(0.62)`}><path className={isSel ? "" : "rblip"} d="M0,-9 L1.6,-2 L9,2 L9,3.6 L1.6,2.4 L1.2,7 L3.4,9 L3.4,10 L0,8.6 L-3.4,10 L-3.4,9 L-1.2,7 L-1.6,2.4 L-9,3.6 L-9,2 L-1.6,-2 Z" fill={isSel ? SEL_GREEN : col} stroke="#0A0E14" strokeWidth="0.6" /></g>
              )}
              {isSel && <text x="10" y="3" fill={SEL_GREEN} fontSize="9" fontFamily="monospace">{a.callsign || a.id}</text>}
            </g>
          );
        })}
        <circle cx="200" cy="200" r="3" fill={C.cyan} />
      </svg>
      <div className="flex items-center justify-between py-2 font-mono" style={{ pointerEvents: "none", paddingRight: 12, paddingLeft: 12, fontSize: 11,
          minHeight: 46, alignItems: "flex-end",
          borderTop: `1px solid ${C.line}`, background: "rgba(10,14,20,0.6)",
